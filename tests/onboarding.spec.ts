...
test('returns to the calling website after all eight lessons', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: /welcome to #welcome-to-hack-club/i })).toBeVisible()
...
test('loads the general Hack Club Slack preset', async ({ page }) => {
  ...
  await expect(page.getByText('Mission complete!')).toBeVisible()
  await page.getByRole('button', { name: /next mission/i }).click()
  await expect(page.getByLabel('Message welcome-to-hack-club')).toBeVisible()
})
